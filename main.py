...
try:
    # 디렉토리에서 모든 JSON 파일 가져오기
    json_files = [f for f in os.listdir(weekly_dir) if f.endswith('.json')]
    # 파일명 기준으로 정렬 (1.json, 2.json, ... 10.json 순서로)
    json_files.sort(key=natural_sort_key)
    print("try문 실행")
    # 각 JSON 파일 읽기
    for json_file in json_files:
        print("json_file", json_file)
        file_path = os.path.join(weekly_dir, json_file)
        with open(file_path, "r", encoding="utf-8") as file:
        ...
        # 해당 번호의 HTML 파일이 있는지 확인하고 내용 불러오기
        html_filename = json_file.replace('.json', '.html')
        html_path = os.path.join(weekly_dir, html_filename)
        if os.path.exists(html_path):
            with open(html_path, "r", encoding="utf-8") as html_file:
                # 파일 번호를 키로 사용 (확장자 제거)
                weekly_number = int(os.path.splitext(json_file)[0])
                weekly_html[weekly_number] = html_file.read()
                print(f"HTML 파일 로드: {html_filename}")
...
@app.get("/weekly")
def weekly_list():  # 함수명 의미 명확하게 수정
    print(weekly_data)
...
def article_detail(weekly_number: int):  # 함수명 의미 명확하게 수정
    # 변수 선언
    back_data = None
    front_data = None
    
    # 유효성 검사 추가
    if weekly_number <= 0 or weekly_number > len(weekly_data):
        raise HTTPException(status_code=404, detail="Article not found")
    
    # HTML 내용 가져오기
    contents = weekly_html.get(weekly_number, "")
    if not contents:
        # HTML 파일이 없으면 직접 로드 시도
        html_path = os.path.join(weekly_dir, f"{weekly_number}.html")
        try:
            with open(html_path, "r", encoding="utf-8") as html_file:
                contents = html_file.read()
                # 캐시에 저장
                weekly_html[weekly_number] = contents
        except FileNotFoundError:
            contents = "<p>HTML 내용이 없습니다.</p>"

    # 이전 글 데이터
    if weekly_number-2 >= 0:
        back_data = weekly_data[weekly_number-2]

    # 다음 글 데이터
    if weekly_number < len(weekly_data):
        front_data = weekly_data[weekly_number]
    ...
    return templates.TemplateResponse(
        "weekly_one.html", 
        {
            "request": {}, 
            "html": contents,
            "data": weekly_data[weekly_number-1],
            "back": back_data,
            "front": front_data,
